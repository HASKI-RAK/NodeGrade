  ...
  it('shows a flagged review output as a warning card with its reason (SPEC-0020/FR-002)', () => {
    render(
      <TaskView
        ...
        outputs={{
          '9': {
            ...output('9', 'review', 'Needs a tutor?', 'The category is not supported.'),
            verdict: 'flagged'
          }
        }}
      />
    ...
    const card = screen.getByRole('article', { name: 'Needs a tutor?' })
    expect(within(card).getByText('Needs review')).toBeVisible()
    expect(within(card).getByText('The category is not supported.')).toBeVisible()
    expect(within(card).getByText('A recommendation, not an approval.')).toBeVisible()
    expect(within(card).queryByText('Passed')).toBeNull()
    expect(
      screen.getByText('Generated by a language model, so it can be wrong or misleading.')
    ).toBeVisible()
  })

  it('shows a clear review output with a neutral chip and no invented reason', () => {
    render(
      <TaskView
        question="Question"
        onSubmit={vi.fn()}
        outputs={{
          '9': { ...output('9', 'review', 'Needs a tutor?', ''), verdict: 'clear' }
        }}
      />
    )

    const card = screen.getByRole('article', { name: 'Needs a tutor?' })
    expect(within(card).getByText('No issue found')).toBeVisible()
    expect(within(card).queryByText('Needs review')).toBeNull()
    expect(within(card).queryByText('Flagged without a reason.')).toBeNull()
    expect(within(card).getByText('A recommendation, not an approval.')).toBeVisible()
  })
